/* ===========================================================
   ...
   Toutes les images : images/avatars/<couche>/<fichier>.png (400×400 transparent).
   Nommage : visage/cheveux = "<genre>_<n>.png" (h_1, f_1…) ; pilosite/accessoire = "<n>.png".
...
const AV_PILOSITE   = 0;               // pilosité (homme)
const AV_ACCESSOIRE = 0;               // accessoires (unisexe)
...
function _avCouches(a){
  ...
  if(a.accessoire) L.push(`accessoire/${a.accessoire}`);
  return L;
}
...
function _renderAvModal(){
  ...
  h+=_avRangee("Pilosité","pilosite", g==="h"?AV_PILOSITE:0, true, g);
  h+=_avRangee("Accessoire","accessoire", AV_ACCESSOIRE, true, g);
  h+=`</div>`;
  m.innerHTML=h;
  _majAvPortrait();
  m.querySelector("[data-fermer]").addEventListener("click", _fermerAvatar);
  m.querySelectorAll("[data-genre]").forEach(b=>b.addEventListener("click",()=>{ if(b.disabled) return; a.genre=b.dataset.genre; a.visage=0; a.cheveux=0; a.pilosite=0; _renderAvModal(); }));
...
function _avRangee(titre, couche, n, avecAucun, g){
  ...
  for(let i=1;i<=n;i++){
    const file = (couche==="visage"||couche==="cheveux") ? `${couche}/${g}_${i}` : `${couche}/${i}`;
...
function _avStyle(){
  ...
  st.textContent=`
    ...
    .av-portrait{ position:relative; width:180px; aspect-ratio:3/4; flex:0 0 auto; border-radius:16px; overflow:hidden; background:radial-gradient(circle at 50% 35%,#123152,#0a1730); border:1px solid var(--edge); display:grid; place-items:center; -webkit-mask-image:linear-gradient(to bottom,#000 74%,transparent 100%); mask-image:linear-gradient(to bottom,#000 74%,transparent 100%); }
    ...
    .av-vig{ position:relative; width:60px; height:80px; flex:0 0 auto; border:2px solid var(--line); border-radius:10px; overflow:hidden; background:#0a1730; cursor:pointer; padding:0; }

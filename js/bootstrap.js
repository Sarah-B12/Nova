...
(async function demarrer(){
  if(SERVEUR_DISPO){
    ...
    if(session){
      ...
      if(prof && prof.donnees && prof.donnees.inscrit){ etat = hydraterEtat(prof.donnees); if(typeof prof.credits==="number"){ etat.credits=prof.credits; if(typeof initCredits==="function") initCredits(prof.credits); } if(typeof prof.reputation==="number") etat.reputation=prof.reputation; etat.roleAdmin=prof.role_admin||null; etat.cercles=prof.cercles||{}; etat.avatar=prof.avatar||null; /* l'avatar vient de la COLONNE profils.avatar, plus de donnees */ afficher(); journal("Systèmes en ligne. Surveille ton énergie."); if(typeof compterNotifs==="function") compterNotifs(); if(typeof compterPoste==="function") compterPoste(); if(typeof compterAnnonce==="function") compterAnnonce(); if(typeof syncPrison==="function") syncPrison(); if(typeof syncEffetsCombat==="function") syncEffetsCombat(); if(!etat.avatar && typeof ouvrirAvatar==="function") ouvrirAvatar({obligatoire:true}); /* rattrapage : inscription interrompue avant le choix */ }
